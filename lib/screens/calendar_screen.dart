...
// ── 圖片顯示 helper：支援 URL 及 base64:... 格式 ────────────────
Widget _buildEventImage(String image, {double? width, double? height, BoxFit fit = BoxFit.cover, Widget? placeholder}) {
  final ph = placeholder ?? Container(
    width: width, height: height,
    color: const Color(0xFFE8E0D8),
    child: const Center(child: Icon(Icons.calendar_month_rounded, size: 32, color: Colors.white54)),
  );
  if (image.isEmpty) return ph;
  if (image.startsWith('base64:')) {
    try {
      final bytes = base64Decode(image.substring(7));
      return Image.memory(bytes, width: width, height: height, fit: fit);
    } catch (_) { return ph; }
  }
  return Image.network(image, width: width, height: height, fit: fit, errorBuilder: (_, __, ___) => ph);
}
        ...
        slivers: [
          ...
          SliverToBoxAdapter(
            child: _buildStyledSectionTitle(
              ...
              _searchQuery.isNotEmpty ? '搜尋結果：$_searchQuery' : _isCalendarView ? '${_selectedDay.month}月${_selectedDay.day}日 的活動' : '所有活動',
            ),
          ),
          if (_isLoading)
          ...
          children: [
            ...
            Expanded(
              child: TextField(
                ...
                onSubmitted: (v) => setState(() { _searchQuery = v.trim(); if (v.trim().isNotEmpty) _isCalendarView = false; }),
            ...
            GestureDetector(
              onTap: () => setState(() { _searchQuery = _searchCtrl.text.trim(); if (_searchQuery.isNotEmpty) _isCalendarView = false; }),
              child: Padding(
                padding: const EdgeInsets.all(6),
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                  decoration: BoxDecoration(color: const Color(0xFF8BAA88), borderRadius: BorderRadius.circular(20)),
                  child: const Text('搜尋', style: TextStyle(fontFamily: 'MyCustomFont', color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
                ),
              ),
            ),
          ],
                  ...
                  child: Row(children: [
                    if (widget.event.image.isNotEmpty)
                      ClipRRect(borderRadius: BorderRadius.circular(8),
                          child: _buildEventImage(widget.event.image, width: 48, height: 48,
                              placeholder: Container(width: 48, height: 48, color: const Color(0xFFE8E0D8))))
                    else
          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              ...
              // 行程：不顯示圖片，改用漸層 + 地圖 icon
                ...
                fit: StackFit.expand,
                children: [
                  _buildEventImage(widget.event.image, fit: BoxFit.cover,
                      placeholder: Container(color: const Color(0xFFE8E0D8),
